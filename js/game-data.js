...
const UNITS = {
  ...
  flameRook: { name:"焰车", family:"车系衍生", unlockAt:5, shape:"rook", cost:200, hp:1050, cd:13, attack:95, attackMode:"flat", burnDps:40, color:"#ef8a52", desc:"灼弹 · 每1.2秒攻击，命中后燃烧3秒，每秒40伤害" },
  ...
  poisonRook: { name:"毒车", family:"车系衍生", unlockAt:6, shape:"rook", cost:200, hp:1050, cd:13, attack:60, attackMode:"flat", poisonDps:30, color:"#9ccc6c", desc:"蚀弹 · 每1.3秒攻击，中毒8秒，每秒造成30伤害" },
  ...
  flameQueen: { name:"焰后", family:"后系衍生", unlockAt:6, shape:"queen", cost:650, hp:900, cd:19, attack:85, attackMode:"flat", element:"flame", projectile:"flameRook", burnDps:35, color:"#ef8650", desc:"三路炎令 · 每1.25秒向连续三路发射火弹，点燃命中的敌军" },
  poisonQueen: { name:"毒后", family:"后系衍生", unlockAt:6, shape:"queen", cost:650, hp:860, cd:19, attack:55, attackMode:"flat", element:"poison", projectile:"poisonRook", poisonDps:25, color:"#94ca64", desc:"三路蛊令 · 每1.35秒向连续三路发射毒弹，优先扩散到未中毒敌军" },
  ...
  poisonBishop: { name:"毒象", family:"象系衍生", unlockAt:6, shape:"bishop", cost:190, hp:620, cd:11, attack:85, attackMode:"arc", poisonDps:24, color:"#9bd36f", desc:"疫线 · 每1.25秒沿斜线投射毒弧，中毒可向召唤援军传播" },
  ...
  superFlameRook: { name:"超级焰车", family:"超级融合种", fusionOnly:true, fusionBase:"superRook", fusionSource:"flameRook", shape:"rook", cost:0, hp:1420, cd:0, attack:85, attackMode:"flat", element:"flame", projectile:"flameRook", burnDps:45, color:"#ff8a4d", desc:"四联熔炉 · 由超级车与焰车融合，每轮4枚火弹持续点燃并熔化冰面" },
  superPoisonRook: { name:"超级毒车", family:"超级融合种", fusionOnly:true, fusionBase:"superRook", fusionSource:"poisonRook", shape:"rook", cost:0, hp:1380, cd:0, attack:55, attackMode:"flat", element:"poison", projectile:"poisonRook", poisonDps:35, color:"#86d85d", desc:"四联菌炮 · 由超级车与毒车融合，每轮4枚毒弹优先感染不同敌军" },
  ...
  superFlameQueen: { name:"超级焰后", family:"超级融合种", fusionOnly:true, fusionBase:"superQueen", fusionSource:"flameQueen", shape:"queen", cost:0, hp:1140, cd:0, attack:85, attackMode:"flat", element:"flame", projectile:"flameRook", burnDps:45, color:"#ff9360", desc:"十二联炎令 · 由超级后与焰后融合，连续三路各发4枚火弹" },
  superPoisonQueen: { name:"超级毒后", family:"超级融合种", fusionOnly:true, fusionBase:"superQueen", fusionSource:"poisonQueen", shape:"queen", cost:0, hp:1100, cd:0, attack:55, attackMode:"flat", element:"poison", projectile:"poisonRook", poisonDps:35, color:"#9be16c", desc:"十二联蛊令 · 由超级后与毒后融合，连续三路各发4枚毒弹" }
};
...
const RANK_TRAITS = {
  ...
  iceRook:[["寒冰弹","冰弹使命中目标减速45%，持续3.5秒。"],["冰面净化","每次开火可融化自身前方最近的一格敌方冰面。"],["冰雕","同一敌人连续被命中3次会被冻结成障碍1.5秒。"],["寒潮逆流","冻结敌人时将其向右推回0.6格。"],["永久冻土","本行敌军越靠近左侧，受到的减速越强，最低只剩25%速度。"]],
  flameRook:[["火焰弹","命中后点燃目标3秒。"],["点火线","灼烧敌人会照亮同格，友军对其攻击频率提高15%。"],["熔冰","火弹经过的冰面会被融化，并留下可再次部署的格子。"],["火种接力","燃烧目标死亡时，火种跳到本路最前方未燃烧敌军。"],["凤凰炉","每累计造成2000灼烧伤害，自动喷出一次整路火浪。"]],
  poisonRook:[["蚀毒弹","毒弹令目标中毒8秒。"],["毒性选择","自动优先攻击尚未中毒的敌人，让毒素铺满队伍。"],["菌丝寄生","中毒敌人被八卦相召唤出的援军靠近时，会把毒传过去。"],["逆疗孢子","中毒会封锁紫蛇和其他敌军的生命回复。"],["孢子傀儡","中毒敌人死亡时留下短命孢子兵，向右阻挡敌军3秒。"]],
...
Object.assign(RANK_TRAITS,{
  iceQueen:[["三路寒令","连续三路各发射一枚减速冰弹。"],["霜层叠加","同一目标连续中弹会积累霜层，第三层冻结1.5秒。"],["五路冬幕","每第3轮改为向全部五路发射冰弹。"],["冰牢敕令","冻结持续时间提高，冻结目标会被推回0.4格。"],["绝对零庭","攻击冻结目标时追加一次完整伤害，并延长冻结。"]],
  flameQueen:[["三路炎令","连续三路各发射一枚燃烧火弹。"],["照明王令","燃烧目标被照亮，受到友军攻击时额外承伤。"],["五路火幕","每第3轮改为向全部五路发射火弹。"],["熔界巡火","火弹会熔化覆盖路线中的冰面。"],["凤凰敕令","每第5轮引爆全场燃烧目标，造成一次额外火伤。"]],
  poisonQueen:[["三路蛊令","连续三路各发射一枚持续中毒的毒弹。"],["择净施毒","每路优先选择尚未中毒的敌军。"],["五路毒幕","每第3轮改为向全部五路扩散毒弹。"],["逆疗王令","中毒同步封锁敌军回复。"],["百蛊议会","场上每名中毒敌军都会略微提高毒后的攻击频率。"]],
  superIceRook:[["四联冰炮","本路同时发射4枚冰弹。"],["分目标冻结","4枚冰弹优先分击不同目标并叠加霜层。"],["双层寒匣","每第2轮在0.15秒后再次完整齐射。"],["寒潮推阵","冻结敌军时将其向右推回0.6格。"],["永冻回响","每轮0.3秒后复奏，冻结目标额外承受一次伤害。"]],
  superFlameRook:[["四联火炮","本路同时发射4枚火弹。"],["分目标点燃","4枚火弹优先点燃不同目标。"],["双层熔匣","每第2轮在0.15秒后再次完整齐射。"],["熔路火控","每轮清除本路前方全部冰面。"],["凤凰回响","每轮0.3秒后复奏，并引爆正在燃烧的目标。"]],
  superPoisonRook:[["四联毒炮","本路同时发射4枚毒弹。"],["分目标染毒","4枚毒弹优先感染不同目标。"],["双层菌匣","每第2轮在0.15秒后再次完整齐射。"],["逆疗菌潮","中毒目标完全停止回复。"],["孢子回响","每轮0.3秒后复奏，击败中毒目标会留下孢子兵。"]],
  superIceQueen:[["十二联冰令","连续三路各同时发射4枚冰弹。"],["分路霜议","每路冰弹优先分击并快速冻结。"],["五路冬幕","每第3轮改为五路各发射4枚冰弹。"],["寒廷协同","五路齐射后推进全部冰系棋子的攻击计时。"],["永冬天幕","每轮0.3秒后按原覆盖路线完整复奏。"]],
  superFlameQueen:[["十二联炎令","连续三路各同时发射4枚火弹。"],["分路火议","每路火弹优先点燃不同目标。"],["五路火幕","每第3轮改为五路各发射4枚火弹。"],["熔廷协同","五路齐射熔化全场冰面并推进焰系棋子。"],["赤日天幕","每轮0.3秒后复奏并引爆燃烧目标。"]],
...
const ABILITY_SPECS = {
  ...
  superIceRook:{kind:"fan",shots:60,perLane:60,rings:5,shotsPerRing:12,ringInterval:.3,duration:1.5,flightSeconds:1.5,damageMin:60,damageMax:80,projectile:"iceRook",element:"ice",fanDegrees:45,lanes:1},superFlameRook:{kind:"fan",shots:60,perLane:60,rings:5,shotsPerRing:12,ringInterval:.3,duration:1.5,flightSeconds:1.5,damageMin:75,damageMax:95,projectile:"flameRook",element:"flame",burnSeconds:3,burnDps:45,fanDegrees:45,lanes:1},superPoisonRook:{kind:"fan",shots:60,perLane:60,rings:5,shotsPerRing:12,ringInterval:.3,duration:1.5,flightSeconds:1.5,damageMin:45,damageMax:65,projectile:"poisonRook",element:"poison",poisonSeconds:8,poisonDps:35,fanDegrees:45,lanes:1},
  superIceQueen:{kind:"fan",shots:180,perLane:60,rings:5,shotsPerRing:12,ringInterval:.3,duration:1.5,flightSeconds:1.5,damageMin:60,damageMax:80,projectile:"iceRook",element:"ice",fanDegrees:45,lanes:3},superFlameQueen:{kind:"fan",shots:180,perLane:60,rings:5,shotsPerRing:12,ringInterval:.3,duration:1.5,flightSeconds:1.5,damageMin:75,damageMax:95,projectile:"flameRook",element:"flame",burnSeconds:3,burnDps:45,fanDegrees:45,lanes:3},superPoisonQueen:{kind:"fan",shots:180,perLane:60,rings:5,shotsPerRing:12,ringInterval:.3,duration:1.5,flightSeconds:1.5,damageMin:45,damageMax:65,projectile:"poisonRook",element:"poison",poisonSeconds:8,poisonDps:35,fanDegrees:45,lanes:3}
};
