...
function renderGrowth(type=growthState.type){
  ...
  $("#growthRoster").innerHTML=orderedUnitEntries().map(([key,item])=>{const unlocked=isUnitUnlocked(key),rank=save.ranks[key]||1,state=item.fusionOnly?'融合限定':unlocked?`${rank}阶`:unlockProgressText(key);return `<button class="growth-unit ${key===type?'active':''} ${unlocked?'':'preview-locked'} ${item.fusionOnly?'fusion-unit':''}" data-growth-unit="${key}" style="--unit-color:${item.color}" title="${item.name} · ${item.family} · ${state}" aria-label="${item.name}，${rank}阶，${state}"><span class="choice-card-art">${unitArt(key,'choice-art')}</span><span class="choice-card-rank">${item.fusionOnly?'融':rank}</span><span class="choice-card-footer growth-card-name">${item.name}</span></button>`;}).join('');
  ...
  $("#growthUnitArt").innerHTML=unitArt(type,'growth-hero-art');$("#growthFamily").textContent=u.family+(u.fusionOnly?` · ${unlockProgressText(type)}`:unlocked?' · 已解锁':` · ${unlockProgressText(type)}`);$("#growthUnitName").textContent=u.name;$("#growthUnitDesc").textContent=u.desc;$("#growthCurrentRank").textContent=u.fusionOnly?'融合限定':`${save.ranks[type]} 阶`;
...
function configureGrowthScenario(state,player,type,rank,mode){
  ...
  if((type==='flameQueen'||type==='superFlameQueen')&&rank>=4)[0,1,2,3,4].forEach(row=>addIce(row,[3,5,7]));
  if(type==='superFlameRook'&&rank>=4)addIce(player.row,[3,4,5,6,7]);
...
function setupGrowthBattle(stage){
  ...
  withBattleContext(state,effectLayer,()=>{
    ...
    else if(mode==='ult'&&UNITS[type].shape==='queen')[1,2,3].forEach(row=>{addGrowthEnemy(state,nextGrowthEnemy(state,type),row,5);addGrowthEnemy(state,nextGrowthEnemy(state,type),row,7);});
    ...
    if(mode==='rank'){const cycle={pawn:3,shieldPawn:4,spearPawn:4,rook:5,twinRook:5,iceRook:3,flameRook:4,poisonRook:4,superRook:2,superIceRook:2,superFlameRook:2,superPoisonRook:2,knight:3,stormKnight:3,frostKnight:3,bishop:3,twinBishop:3,iceBishop:3,flameBishop:3,poisonBishop:4,prismQueen:5}[type];if(cycle)player.traitHits=cycle-1;if(type==='queen'&&rank===3)player.attackCycles=2;if(type==='twinQueen'&&rank===3)player.attackCycles=3;if(['iceQueen','flameQueen','poisonQueen'].includes(type)&&rank>=3)player.attackCycles=2;if(isSuperQueenType(type)&&rank>=3)player.attackCycles=2;configureGrowthScenario(state,player,type,rank,mode);}
